    ...
    <>
      <nav className="fixed top-0 w-full z-[60] flex items-center justify-between px-6 md:px-12 py-4 bg-[var(--cream)]/90 backdrop-blur-md border-b border-[var(--gold)]/10">
        ...
        <Link href="/" className="flex items-center gap-3 absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0">
          <Image src="/logo.png" alt="Origin 54" width={42} height={42} className="rounded-full border border-[var(--gold)]/20" priority />
        ...
        <button 
          ...
          className="relative p-2 text-[var(--charcoal)]"
        ...
        <div className="fixed inset-0 z-[70] bg-[var(--charcoal)] flex flex-col px-8 pt-16 pb-10">
          ...
          <nav className="flex flex-col gap-8 mt-8">
            {[["Shop", "/shop"], ["Artisans", "/artisans"], ["Our Story", "/about"]].map(([label, href]) => (
              <Link key={href} href={href} onClick={() => setIsMenuOpen(false)} className="font-display text-4xl text-[var(--cream)] uppercase tracking-widest hover:text-[var(--gold)] transition-colors">
                {label}
              </Link>
            ))}
          </nav>
        ...
        <div className="fixed inset-0 z-[70] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-[var(--cream)] h-full flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--gold)]/20">
              <h2 className="font-display text-xl uppercase tracking-widest text-[var(--charcoal)]">Your Bag ({itemCount})</h2>
              <button onClick={() => setIsCartOpen(false)} className="text-[var(--charcoal)]"><X size={22} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                  ...
                  <button onClick={() => setIsCartOpen(false)} className="text-[10px] font-bold tracking-widest uppercase text-[var(--gold)] border border-[var(--gold)] px-6 py-2 hover:bg-[var(--gold)] hover:text-white transition-colors">
                    Continue Shopping
                  </button>
                </div>
              ) : (
                items.map((item) => (
                  <div key={item.id} className="flex gap-4">
                    <div className="relative w-20 h-24 flex-shrink-0 bg-[var(--charcoal)]/5 overflow-hidden">
                      <Image src={item.image_url || "/logo.png"} alt={item.name} fill className="object-cover" sizes="80px" />
                    </div>
                    <div className="flex-1">
                      <p className="font-serif text-sm font-medium text-[var(--charcoal)]">{item.name}</p>
                      <p className="font-serif text-xs text-[var(--charcoal)]/50 mb-2">${item.price}</p>
                      <div className="flex items-center gap-3">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="text-[var(--charcoal)]/60 hover:text-[var(--charcoal)]"><Minus size={14} /></button>
                        <span className="text-sm font-bold w-4 text-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="text-[var(--charcoal)]/60 hover:text-[var(--charcoal)]"><Plus size={14} /></button>
                      </div>
                    </div>
                    <p className="font-serif text-sm font-semibold text-[var(--charcoal)]">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))
              )}
            </div>
            ...
            {items.length > 0 && (
              <div className="px-6 py-6 border-t border-[var(--gold)]/20 space-y-4">
                <div className="flex justify-between font-serif text-sm">
                  ...
                  <span className="font-semibold text-[var(--charcoal)]">${total.toFixed(2)}</span>
                </div>
                <Link href="/checkout" onClick={() => setIsCartOpen(false)} className="flex items-center justify-center gap-2 w-full bg-[var(--charcoal)] text-[var(--cream)] py-4 text-[10px] font-bold tracking-[0.2em] uppercase hover:bg-[var(--gold)] transition-colors">
